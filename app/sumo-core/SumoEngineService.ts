import UntypedEngine from './SumoEngine';

const UPDATE_BG_STATE_KEY = 'SUMO_UPDATE_BG_STATE';
const INIT_BG_STATE_KEY = 'SUMO_INIT_BG_STATE';

class SumoEngineService {
  private engineInitialized = false;

  /**
   * Initializer for the SumoEngineService
   *
   * @param store - Redux store
   */
  initalizeEngine = (store: any) => {
    const reduxState = store.getState?.();
    const state = reduxState?.sumoEngine?.backgroundState || {};
...
/**
 * SumoEngineService class used for initializing and subscribing to the engine controllers
 */
export default new SumoEngineService();
