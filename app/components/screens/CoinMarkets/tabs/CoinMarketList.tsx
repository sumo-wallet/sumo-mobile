...
import { RawCoinMarketsInterface } from '../../../../reducers/coinmarkets/types';
...
export interface CoinMarketListInterface {
  params: MarketsListParams;
  onUpdateParams: (value: MarketsListParams) => void;
  loading: boolean;
  call: () => Promise<RawCoinMarketsInterface[]>;
}

export const CoinMarketList = memo(
  ({ params, onUpdateParams, loading, call }: CoinMarketListInterface) => {
    const { colors } = useTheme();
    const styles = createStyles(colors);
    const coinIds = getCoinMarketsByQuery('all');
    const globalMarket = useCoinMarketsByQuery('global_market');

    const [{ loading: loadingInterval }, callInterval] =
      useAsyncFn(async () => {
    ...
    const onReload = useCallback(async () => {
      onUpdateParams(Object.assign({}, params, { page: 1 }));
    }, [onUpdateParams, params]);

    // eslint-disable-next-line @typescript-eslint/no-shadow
    const paramsCustom = useCallback(
      (value: MarketsListParams) => {
        onUpdateParams(Object.assign({}, params, value));
      },
      [onUpdateParams, params],
    );
    ...
    const updateParams = useCallback(async () => {
      ...
      if (data && data.length >= 20) {
        onUpdateParams(
          Object.assign({}, params, { page: (params?.page || 0) + 1 }),
        );
      }
    }, [call, onUpdateParams, params]);
